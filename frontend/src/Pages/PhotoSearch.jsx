...
export default function PhotoSearch() {
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, var(--abyss) 0%, var(--navy) 40%, var(--abyss) 100%)', paddingTop: '80px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', width: '500px', height: '500px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(220,163,163,0.15) 0%, transparent 70%)', top: '10%', right: '10%', pointerEvents: 'none' }} />
            ...
            <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 24px', position: 'relative', zIndex: 2 }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: 'center', marginBottom: '48px' }}>
                    <div className="section-tag glass-pink" style={{ color: 'var(--dusty-pink)', border: 'none' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: preview ? '1fr 1fr' : '1fr', gap: '24px' }}>
                    ...
                    <div>
                        <div
                            ...
                            className={dragOver ? "glass-pink deep-hover" : "glass deep-hover"}
                            style={{
                                border: `2px dashed ${dragOver ? 'var(--dusty-pink)' : 'rgba(123, 189, 232, 0.3)'}`,
                                borderRadius: '20px',
                                padding: '40px',
                                textAlign: 'center',
                                cursor: 'pointer',
                        ...
                        {preview && (
                            <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                <input value={userText} onChange={e => setUserText(e.target.value)}
                                    ...
                                    style={{ background: 'rgba(10, 65, 116, 0.4)', borderColor: 'rgba(123, 189, 232, 0.3)', color: '#FBE4D8' }} />
                        ...
                        {result && !loading && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                                {result.mode === 'location_identified' && result.location && (
                                    <div className="glass-pink deep-hover" style={{ padding: '28px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                                            <MapPin size={16} color="var(--dusty-pink)" />
                                            <span style={{ color: 'var(--dusty-pink)', fontSize: '12px', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase' }}>Location identified</span>
                                        </div>
                                        ...
                                        <p style={{ color: 'var(--dusty-pink)', fontSize: '14px', marginBottom: '16px' }}>📍 {result.location.country}</p>
                                ...
                                {result.mode === 'vibe_recommendation' && result.recommendation && (
                                    <div className="glass-pink deep-hover" style={{ padding: '28px' }}>
                                        <div style={{ color: 'var(--dusty-pink)', fontSize: '12px', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '8px' }}>✨ Vibe match</div>
                                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '20px' }}>
                                            {[result.vibe?.primary_vibe, ...(result.vibe?.aesthetic_tags || [])].filter(Boolean).map((t, i) => (
                                                <span key={i} style={{ background: 'rgba(220,163,163,0.2)', color: 'var(--dusty-pink-glow)', fontSize: '12px', padding: '4px 12px', borderRadius: '50px' }}>{t}</span>
                                            ))}
                                        </div>
                                        ...
                                        <p style={{ color: 'var(--dusty-pink)', fontSize: '14px', marginBottom: '16px' }}>📍 {result.recommendation.country}</p>
